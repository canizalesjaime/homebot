# To Do
1. homebot: urdf file, fix tennis navigation, add camera in ros2 to docker, check the c++ camera packages, CSI camera support in Docker is tricky; often easier to stream the camera via host and have the ROS node access /dev/video0 inside the container. segmentation, stereo camera, lidar slam for 3d stuff  
2. House keeping: fix servo combo,add keystrokes to website for motion, database for objects seen with dates(web dev course), print in autocad plexi connectors, connect i2c to powerboard for use by pca9865 and mpu6050, automate git push for ci/cd, aws? separate git for robot? add notes for electronics
