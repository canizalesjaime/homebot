# To Do
1. homebot: urdf file, add yolo, fix tennis navigation, add camera in ros2 to docker, check the c++ camera packages, CSI camera support in Docker is tricky; often easier to stream the camera via host and have the ROS node access /dev/video0 inside the container. segmentation, stereo camera, ultrasound slam, lidar slam for 3d stuff 
